from django.contrib import admin


class CrfMetaDataAdminMixin(admin.ModelAdmin):

    search_fields = (
        'subject_identifier', 'model', 'id')
    list_display = (
        'subject_identifier', 'model', 'visit_code', 'entry_status', 'fill_datetime',
        'due_datetime', 'close_datetime', 'created', 'hostname_created')
    list_filter = (
        'entry_status', 'visit_code', 'schedule_name', 'visit_schedule_name',
        'model', 'fill_datetime', 'created', 'user_created', 'hostname_created')
    readonly_fields = ('subject_identifier', 'model', 'visit_code',
                       'schedule_name', 'visit_schedule_name',
                       'show_order', 'current_entry_title')


class RequisitionMetaDataAdminMixin:

    search_fields = (
        'subject_identifier', 'model', 'id', 'panel')
    list_display = (
        'subject_identifier', 'model', 'panel', 'visit_code', 'entry_status', 'fill_datetime',
        'due_datetime', 'close_datetime', 'created', 'hostname_created')
    list_filter = (
        'entry_status', 'panel', 'visit_code', 'schedule_name', 'visit_schedule_name',
        'model', 'fill_datetime', 'created', 'user_created', 'hostname_created')
    readonly_fields = ('subject_identifier', 'model', 'visit_code',
                       'schedule_name', 'visit_schedule_name',
                       'show_order', 'current_entry_title')
