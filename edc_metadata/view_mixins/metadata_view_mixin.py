    ...
    def get_crfs(self, **kwargs):
        ...
        if self.appointment:
            ...
            for metadata in self.crf_metadata_set:
                try:
                    model_cls = django_apps.get_model(metadata.model)
                except LookupError as e:
                    self.handle_lookup_error(metadata)
                options = {
                    f'{model_cls.visit_model_attr()}': visit}
                try:
                    model_obj = model_cls.objects.get(**options)
